...
import { MessageSquare, Heart, Send, Plus, Search, CornerDownRight, Image as ImageIcon, Link as LinkIcon, ChevronDown, ChevronUp, X, ExternalLink, Edit2, Trash2, Save } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-3xl mx-auto fade-in-up pb-20">
      ...
      <div className="space-y-6">
          {/* Section: My Posts Toggle */}
          <div className="flex justify-between items-center px-2">
              <h3 className="text-xs font-black uppercase tracking-widest text-stone-300">Community Feed</h3>
              <button 
              ...
              >
                  {isMyPostsOpen ? 'Show All' : 'My Posts Only'}
              </button>
          </div>
          ...
          {(isMyPostsOpen ? myPosts : filteredPosts).map(post => {
              ...
              <div key={post.id} className="bg-white p-6 rounded-[2rem] border border-stone-100 shadow-sm hover:shadow-md transition-shadow group">
                  ...
                  <div className="flex gap-2 mb-4 flex-wrap">
                      {post.tags.map(tag => (
                          <span key={tag} className="text-[9px] font-bold uppercase tracking-wider bg-stone-50 text-stone-400 px-2 py-1 rounded-md border border-stone-100">
                              #{tag}
                          </span>
                      ))}
                  </div>
